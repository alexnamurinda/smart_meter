...
if (isset($_GET['action'])) {
    ...
    if ($action === 'update_power_status' && isset($_POST['room_id']) && isset($_POST['power_status'])) {
        ...
        try {
            ...
            header('Content-Type: application/json');
            echo json_encode(['success' => $result, 'status' => $powerStatus]);
            exit;
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Failed to update power status']);
            exit;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        table#roomsTable tr:hover {
            background-color: #f1f1f1;
            cursor: pointer;
        }

        table#roomsTable tr.selected {
            background-color: #007BFF;
            color: white;
        }
            ...
            fetch(`overview.php?action=fetch_rooms&apartment_id=${apartmentId}`)
                ...
                .then(data => {
                    data.forEach(room => {
                        ...
                        row.innerHTML = `
                        <td>${room.name}</td>
                        <td>${room.power_status}</td>
                    `;
                        row.addEventListener('click', () => {
                            fetchRoomDetails(room.room_id);
                            // Highlight selected row
                            document.querySelectorAll('#roomsTable tr').forEach(tr => tr.classList.remove('selected'));
