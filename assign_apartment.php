    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['apartment_id'], $_POST['apartment_name'])) {
            ...
            if (!empty($_POST['update_apartment'])) {
                $stmt = $pdo->prepare("UPDATE apartments SET name=? WHERE apartment_id=?");
                $stmt->execute([$_POST['apartment_name'], $_POST['apartment_id']]);
                $message = "Apartment updated successfully!";
            } else {
                // Add new apartment
                $stmt = $pdo->prepare("INSERT INTO apartments (apartment_id, name) VALUES (?, ?)");
        ...
        if (isset($_POST['room_id'], $_POST['room_name'], $_POST['apartment_id'])) {
            $stmt = $pdo->prepare("INSERT INTO rooms (room_id, apartment_id, name) VALUES (?, ?, ?)");
        ...
        if (isset($_POST['delete_apartment_id'])) {
            $stmt = $pdo->prepare("DELETE FROM apartments WHERE apartment_id = ?");
...
<head>
    ...
    <style>
        ...
        .apartment-list, .room-list {
            margin-top: 10px;
        }
        .apartment-item, .room-item {
            padding: 10px;
            background: #f9f9f9;
            margin: 5px 0;
            border-radius: 5px;
