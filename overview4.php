            ...
            fetch(`overview3.php?action=fetch_room_details&room_id=${roomId}`)
                ...
                .then(room => {
                    ...
                    Plotly.newPlot('remainingUnitsGauge', [{
                        ...
                        number: { font: { size: 35 }, suffix: "<span style='font-size:14px;'>kWh</span>", valueformat: ".2f" },
                        gauge: {
                            axis: { range: [0, 100], tickfont: { size: 15 } },
                            steps: [{ range: [0, 50], color: "green" },
                                    { range: [50, 100], color: "green" }],
                            bar: { color: "#f6921e", thickness: 0.5 }
                        }
                    ...
                    const remainingUnits = room.remaining_units;

                    // Initialize power status and toggle state
                    if (room.power_status === 'ON' && remainingUnits >= 0.1) {
                        powerToggle.checked = true;
                        powerStatusElement.textContent = "Status: ON";
                    } else {
                        powerToggle.checked = false;
                        powerStatusElement.textContent = "Status: OFF";
                    }

                    // Toggle Power Status (Change in front-end, update in the backend)
                    powerToggle.addEventListener('change', function() {
                        const powerStatus = powerToggle.checked ? 'ON' : 'OFF';
                        if (remainingUnits < 0.1 && powerToggle.checked) {
                            alert('Remaining units are low. Power will be turned off.');
                            powerToggle.checked = false;
                            return;
                        }
                        fetch('overview3.php?action=update_power_status', {
                            method: 'POST',
                            body: new URLSearchParams({ room_id: room.room_id, power_status: powerStatus }),
                            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                        })
                        .then(response => response.json())
                        .then(data => {
                            powerStatusElement.textContent = "Status: " + data.power_status;
                        });
                    });
                })
                .catch(err => alert("Failed to fetch room details."));
        }
    </script>
</body>
</html>
